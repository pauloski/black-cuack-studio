...
(function () {
  ...
  function panelsHTML() {
    ...
    // Paso 2 — Envío
    var p2 = '<div class="co-panel" data-step="2"><h2>¿A dónde lo enviamos?</h2>' +
      '<p class="co-sub">Despacho a domicilio con Chilexpress.</p>' +
      '<div class="co-ship-box"><i data-lucide="info"></i><div>El costo del envío se calcula con Chilexpress según tu comuna. Elígela y lo verás al instante.</div></div>' +
      '<div class="co-row">' +
        field('region', 'Región', { type: 'select' }) +
        field('comuna', 'Comuna', { type: 'select', attrs: 'disabled' }) +
      '</div>' +
      field('direccion', 'Dirección — calle y número', { autocomplete: 'street-address', ph: 'Av. Los Carrera 1234, depto 5B' }) +
  ...
  async function requestQuote() {
    ...
    try {
      var res = await fetch('/api/shipping/quote', {
        ...
        body: JSON.stringify({ comuna: comuna, items: lineItems() })
      });
  ...
  function validateStep2() {
    ...
    ['comuna', 'direccion'].forEach(function (n) { setErr(n, ''); });
    ...
    if (ok && !shipping.ok) { setFormErr('_step2', 'Espera la cotización del envío (o elige una comuna con cobertura).'); ok = false; }
    return ok;
  }
  ...
  function renderRecap() {
    ...
    var rows = [
      ...
      { k: 'Envío', v: esc(val('direccion')) + '<br>' + esc(val('comuna')) + (val('referencia') ? '<br><span style="color:#9a9a9a">' + esc(val('referencia')) + '</span>' : ''), edit: 2 },
      { k: 'Despacho', v: 'Chilexpress ' + esc(shipping.servicio) + ' · ' + money(shipping.cost), edit: 2 }
    ];
    ...
    try {
      var res = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({
          ...
          shipping: { nombre: val('nombre'), rut: val('rut'), telefono: val('telefono'), direccion: val('direccion'), comuna: val('comuna'), referencia: val('referencia') }
        })
      });
      var data = await res.json().catch(function () { return null; });
  ...
  (async function init() {
    ...
    await Promise.all([loadProducts(), loadComunas()]);
    buildLines(cart);
